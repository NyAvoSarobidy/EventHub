...
export default function Event (){
    ...
    const filteredImages = images.filter(image => {
        ...
        return (
            image.titre.toLowerCase().includes(query) ||
            image.date.toLowerCase().includes(query) ||
            image.type.toLowerCase().includes(query) ||
            image.lieu.toLowerCase().includes(query) ||
            image.horaire.toLowerCase().includes(query) ||
    ...
    const totalPages = Math.ceil(images.length / eventsPerPage);
    const indexOfLastEvent = currentPage * eventsPerPage;
    const indexOfFirstEvent = indexOfLastEvent - eventsPerPage;
    const currentEvents = images.slice(indexOfFirstEvent, indexOfLastEvent);
                    ...
                    {searchQuery && (
                        <div className="text-center mb-6">
                            ...
                            <p className="text-sm text-gray-500 mt-1">
                                {filteredImages.length} événement(s) trouvé(s)
                            </p>
                        </div>
                    )}
                    ...
                    {filteredImages.length > 0 ? (
                        <>
                            <div className="text-center mb-6 text-gray-600">
                                Affichage de {indexOfFirstEvent + 1} à {Math.min(indexOfLastEvent, filteredImages.length)} sur {filteredImages.length} événements
                            </div>

                            {/* Grille d'événements */}
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {currentEvents.map((image, index) => (
                                <div
                                    key={indexOfFirstEvent + index}
